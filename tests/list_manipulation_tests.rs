mod list_manipulation_tests {
    ...
    fn can_remove_duplicate_words() {
        let new_list = tidy_list(make_lists().0, false, false, None);
    ...
    fn can_sort_words_alphabetically() {
        let new_list = tidy_list(make_lists().1, true, false, None);
    ...
    fn removes_blank_lines() {
        let new_list = tidy_list(make_lists().1, false, false, None);
        assert!(new_list.len() == make_lists().1.len() - 2);
    }

    #[test]
    fn can_lowercase_words() {
        let new_list = tidy_list(make_lists().0, true, false, None);
    ...
    fn can_remove_prefix_words() {
        let new_list = tidy_list(make_lists().0, true, true, None);
    ...
    fn can_remove_reject_words() {
        ...
        let new_list = tidy_list(make_lists().1, true, true, Some(words_to_reject));
